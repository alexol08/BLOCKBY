...
const SOURCING_DATA_VERSION = "merged-local-exact-plus-live-links-v3";
...
if (state.sourcing && state.sourcing.dataVersion !== SOURCING_DATA_VERSION) {
  state.sourcing = null;
  saveLocal("blockyby.sourcing", null);
}
...
function sourcePriceLabel(source) {
  ...
  if (source.priceConfidence === "exact") {
    return source.priceText || formatMoney(source.unitPrice, source.currency || "EUR");
  }
  return source.priceText || "Price needs review";
}

function sourceShippingLabel(source) {
  if (!source) return "Unknown";
  if (source.shippingText) return source.shippingText;
  const shipping = Number(source.shippingEstimate || 0);
  if (source.shippingConfidence === "not_included") return "Not included in demo checkout";
  if (Number.isFinite(shipping) && shipping > 0) return formatMoney(shipping, source.currency || "EUR");
  return "Review with supplier";
}

function sourceTotalLabel(source) {
  if (!source) return "Unknown";
  const total = Number(source.totalPriceEstimate || 0);
  if (source.priceConfidence === "exact" && Number.isFinite(total) && total > 0) {
    return formatMoney(total, source.currency || "EUR");
  }
  return "Review before ordering";
}

function linkIsHttp(url) {
  return /^https?:\/\//i.test(String(url || ""));
}

function sourceLinks(source) {
  const links = [];
  if (linkIsHttp(source.productUrl)) links.push(["Product", source.productUrl]);
  if (linkIsHttp(source.datasheetUrl)) links.push(["Datasheet", source.datasheetUrl]);
  if (linkIsHttp(source.sourceSearchUrl)) links.push(["Supplier search", source.sourceSearchUrl]);
  if (!links.length) return "";
  return `<div class="source-links">${links.map(([label, url]) => `<a href="${escapeHtml(url)}" target="_blank" rel="noreferrer">${escapeHtml(label)}</a>`).join(" ")}</div>`;
}

function sourceLinkStatus(source) {
  const product = source.linkVerification?.product;
  if (!product) return source.productUrlVerified ? "Product link checked" : "Open supplier search to review live availability";
  if (product.ok) return product.status ? `Product link checked (${product.status})` : "Product link checked";
  return product.message || "Product link not checked";
}

function renderSourceCandidates(item) {
  const sources = item.sourceCandidates || [];
  if (!sources.length) {
    const owned = item.sourceStatus === "owned";
    return `
      <div class="source-card ${owned ? "selected" : "unverified"}">
        <h4>${owned ? "Already owned" : "No source selected"}</h4>
        <p>${owned ? "This item is marked as available from your profile stock. Still check compatibility before using it." : "This item cannot be checked out until it has a verified source candidate."}</p>
      </div>
    `;
  }

  return sources.map((source, index) => {
    const selected = index === Number(item.selectedSourceIndex ?? -1);
    const verified = source.verificationStatus === "verified";
    const statusClass = verified ? "ok" : "warn";
    const links = sourceLinks(source);
    return `
      <div class="source-card ${selected ? "selected" : ""} ${verified ? "" : "unverified"}">
        <div class="source-head">
          <h4>${escapeHtml(source.productName || "Source candidate")}</h4>
          <span class="pill ${statusClass}">${escapeHtml(source.verificationStatus || "unverified")}</span>
        </div>
        <p>${escapeHtml(source.description || "No description supplied.")}</p>
        <dl>
          <dt>Supplier</dt><dd>${escapeHtml(source.supplier || "Unknown")}</dd>
          <dt>Source type</dt><dd>${escapeHtml(source.sourceType || "catalogue")}</dd>
          ...
          <dt>Unit price</dt><dd>${escapeHtml(sourcePriceLabel(source))}</dd>
          <dt>Shipping</dt><dd>${escapeHtml(sourceShippingLabel(source))}</dd>
          <dt>Selected total</dt><dd>${escapeHtml(sourceTotalLabel(source))}</dd>
          <dt>Stock</dt><dd>${escapeHtml(String(source.stockAvailable ?? "Review live"))}</dd>
          <dt>Evidence</dt><dd>${escapeHtml(source.evidenceNotes || source.verificationMessage || "Matched by source database.")}</dd>
          <dt>Link check</dt><dd>${escapeHtml(sourceLinkStatus(source))}</dd>
  ...
  out.innerHTML = `
    <div class="sourcing-summary">
      <div class="item-top">
        ...
        <span class="pill ${verification.checkoutReady ? "ok" : "warn"}">${verification.checkoutReady ? "checkout ready" : "needs review"}</span>
      </div>
      <p>${escapeHtml(plan.summary || "")}</p>
      <div class="pill-row">
        <span class="pill ok">${verification.verifiedCount ?? 0} verified</span>
        <span class="pill ${verification.unverifiedCount ? "warn" : "ok"}">${verification.unverifiedCount ?? 0} needs review</span>
        <span class="pill">Exact source total: ${formatMoney(plan.estimatedTotal, plan.currency || "EUR")}</span>
        <span class="pill">${escapeHtml(verification.sourceMode || "source database")}</span>
        <span class="pill">Data: ${escapeHtml(plan.dataVersion || "legacy")}</span>
      </div>
      ${verification.pricingNote ? `<p class="note">${escapeHtml(verification.pricingNote)}</p>` : ""}
        ...
        <article class="bom-item ${item.verificationStatus !== "verified" && item.sourceStatus !== "owned" ? "blocked" : ""}">
          <div>
            <div class="item-top">
              ...
              <span class="pill ${item.sourceStatus === "owned" ? "ok" : item.verificationStatus === "verified" ? "ok" : "warn"}">${escapeHtml(item.sourceStatus === "owned" ? "owned" : item.verificationStatus)}</span>
            </div>
            <p class="item-meta">
              Category: ${escapeHtml(item.category)}<br>
              Qty: ${escapeHtml(item.quantity)} · Planning estimate: ${formatMoney(item.unitPriceEstimate, plan.currency || "EUR")} · Selected source: ${formatMoney(item.totalPriceEstimate || 0, plan.currency || "EUR")}<br>
...
function canCheckout(items) {
  ...
  for (const item of items) {
    ...
    if (!item.sourceCandidates || !item.sourceCandidates.length) {
      problems.push(`${item.name}: no source candidate`);
      continue;
    }
    const selectedIndex = Number(item.selectedSourceIndex ?? -1);
    if (selectedIndex < 0 || selectedIndex >= item.sourceCandidates.length) {
      problems.push(`${item.name}: no source selected`);
      continue;
    }
    const source = item.sourceCandidates[selectedIndex];
    if (source.verificationStatus !== "verified") problems.push(`${item.name}: selected source is not verified`);
    if (source.priceConfidence !== "exact" || Number(source.unitPrice || 0) <= 0) problems.push(`${item.name}: selected source has no exact price`);
  }
...
function syncHelpPromptFromPage(force = false) {
  ...
  const prompt = page.helpPrompt || `Help me with page ${page.pageNumber}: ${page.title}`;
  if (force || !input.value.trim() || input.dataset.autofilled === "page") {
    input.value = prompt;
    input.dataset.autofilled = "page";
  }
}

function formatHelpAnswer(data) {
  if (!data.safe) {
    return data.diagnosis || "The helper blocked this question for safety.";
  }
  const lines = [];
  if (data.diagnosis) lines.push(data.diagnosis);
  if (Array.isArray(data.nextChecks) && data.nextChecks.length) {
    lines.push("\nNext checks:");
    data.nextChecks.forEach((item) => lines.push(`• ${item}`));
  }
  if (Array.isArray(data.likelyCauses) && data.likelyCauses.length) {
    lines.push("\nLikely causes:");
    data.likelyCauses.forEach((item) => lines.push(`• ${item}`));
  }
  if (data.recommendedAction) lines.push(`\nRecommended action: ${data.recommendedAction}`);
  if (data.whenToStop) lines.push(`\nWhen to stop: ${data.whenToStop}`);
  if (data.suggestedQuestion) lines.push(`\nGood follow-up: ${data.suggestedQuestion}`);
  if (data._aiError) lines.push(`\nAI fallback used: ${data._aiError}`);
  return lines.join("\n").trim() || "Try one reversible check at a time and compare against the previous working step.";
}
...
function renderHelpChat() {
  ...
  if (!state.helpMessages.length) {
    log.innerHTML = `<div class="empty-state">No build-help messages yet. Generate instructions, then ask about the current page.</div>`;
    syncHelpPromptFromPage(false);
    return;
  }
...
async function askBuildHelp(usePagePrompt = false) {
  const input = $("#helpInput");
  if (!input) return;
  if (usePagePrompt) syncHelpPromptFromPage(true);
  const question = input.value.trim();
  if (!question) {
    toast("Ask the helper a question first.", "warn");
    return;
  }
  const button = $("#helpAskBtn");
  ...
  try {
    ...
    const result = await api("/api/help", {
      ...
      context: {
        ...
        page,
        previousMessages: state.helpMessages.slice(-8)
      }
    });
    state.helpMessages.push({ role: "assistant", content: formatHelpAnswer(result) });
    saveLocal("blockyby.helpMessages", state.helpMessages);
    renderHelpChat();
    input.value = "";
    input.dataset.autofilled = "";
    toast(result.safe === false ? "Build help blocked by safety gate." : "Build help answered.", result.safe === false ? "warn" : "ok");
  } catch (error) {
    toast(error.message, "bad");
  } finally {
    setBusy(button, false);
...
function clearHelpChat() {
  ...
  renderHelpChat();
  syncHelpPromptFromPage(true);
  toast("Build help chat cleared.", "warn");
}
...
function bindEvents() {
  ...
  $("#helpAskBtn")?.addEventListener("click", () => askBuildHelp(false));
  $("#helpPromptBtn")?.addEventListener("click", () => { syncHelpPromptFromPage(true); toast("Current page prompt copied into the helper."); });
  $("#helpClearBtn")?.addEventListener("click", clearHelpChat);
  $("#helpInput")?.addEventListener("input", () => { $("#helpInput").dataset.autofilled = ""; });
