...
for token in ["helpInput", "helpAskBtn", "Simulation lab", "Verified sourcing"]:
    if token not in index_html:
        raise SystemExit(f"Expected UI token missing from index.html: {token}")
for token in ["SOURCING_DATA_VERSION", "askBuildHelp", "sourceLinks", "renderSourceCandidates"]:
    if token not in app_js:
        raise SystemExit(f"Expected UI token missing from app.js: {token}")
