...
from backend.safety import evaluate_action_safety
...
def fallback_project_brief(request: ProjectRefineRequest) -> dict[str, Any]:
    ...
    safety = evaluate_action_safety("plan", request.idea, profile, [dump_model(m) for m in request.messages])
    if not safety.safe:
        return {
            "safe": False,
...
def refine_project(request: ProjectRefineRequest) -> dict[str, Any]:
    safety = evaluate_action_safety("plan", request.idea, normalize_profile(request.profile), [dump_model(m) for m in request.messages])
    if not safety.safe:
        return fallback_project_brief(request)

    instructions = "\n".join([
        "You are Blockyby's safe project-planning agent for hobbyists.",
        "Validate feasibility, ask useful clarifying questions, and optimise around owned tools/materials and a cheap-to-best slider.",
        "Do not help with unsafe, restricted, or age-restricted projects. Return only the provided JSON schema.",
    ])
    input_data = {
        "idea": request.idea,
        "profile": normalize_profile(request.profile),
    ...
    try:
        result = call_openai_json(instructions=instructions, input_data=input_data, schema=PROJECT_BRIEF_SCHEMA)
...
def create_instructions(request: InstructionsRequest) -> dict[str, Any]:
    ...
    instructions = "\n".join([
        ...
        "Focus on reversible prototyping, source verification, compatibility checks, and safe testing.",
        "Return only the provided JSON schema.",
    ])
    input_data = {"profile": request.profile, "brief": request.brief, "sourcing": request.sourcing}
    try:
        result = call_openai_json(instructions=instructions, input_data=input_data, schema=INSTRUCTION_SCHEMA)
...
def create_order(request: OrderRequest) -> dict[str, Any]:
    ...
    order = {
        ...
        "message": "Mock order created. This demo does not place a real supplier/payment order.",
        "itemCount": len(items),
